...
def mostrar_detalhes_projetos(email):
    ...
    # Função para atualizar o relatório do projeto
    def atualizar_relatorio(projeto, nova_info):
        if 'relatorio' not in lider['projetos'][projeto] or not isinstance(lider['projetos'][projeto]['relatorio'], dict):
            lider['projetos'][projeto]['relatorio'] = {}

        numero_semana = len(lider['projetos'][projeto]['relatorio']) + 1
        lider['projetos'][projeto]['relatorio'][f'Semana {numero_semana}'] = nova_info
        sg.popup(f"Relatório da Semana {numero_semana} do projeto {projeto} atualizado.")
        database.salvar_usuarios(usuarios_db)
    ...
    layout = [
        ...
        [sg.Text('Adicionar Informação Semanal:'), sg.Button('Adicionar Relatório')],
    ...
    while True:
        ...
        if event == '-PROJETO-':
            ...
            # Verificar se o projeto selecionado tem um relatório, se não, criar um
            if 'relatorio' not in lider['projetos'][projeto_selecionado] or not isinstance(lider['projetos'][projeto_selecionado]['relatorio'], dict):
                lider['projetos'][projeto_selecionado]['relatorio'] = {}
                database.salvar_usuarios(usuarios_db)
        ...
        if event == 'Adicionar Relatório':
            projeto = values['-PROJETO-']
            if not projeto:
                sg.popup("Por favor, selecione um projeto.")
                continue

            nova_info = sg.popup_get_text("Adicionar Informação Semanal")
            if nova_info:
                atualizar_relatorio(projeto, nova_info)
                # Atualizar a lista de detalhes
                atualizar_detalhes_projetos(lider, window, projetos_detalhes)
...
def atualizar_detalhes_projetos(lider, window, projetos_detalhes):
    ...
    for proj, info_projeto in lider['projetos'].items():
        ...
        if 'relatorio' in info_projeto and isinstance(info_projeto['relatorio'], dict):
            for semana, relatorio in info_projeto['relatorio'].items():
                detalhes_projeto.append(f"{semana}: {relatorio}")
